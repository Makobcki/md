...
def _config_text_enabled(cfg: TrainConfig) -> bool:
    ...
    TrainConfig in this project is intentionally a flat runtime dataclass.
    Nested KDL sections such as ``text.enabled`` are kept in ``cfg.extra`` unless
    they are explicitly flattened into a dataclass field. Do not access
    ``cfg.text`` here; it does not exist for the flat runtime config.

    Resolution order:
    1. KDL ``text.enabled`` from ``cfg.extra``;
...
def _main_impl(argv: list[str] | None = None, cfg_override: TrainConfig | None = None) -> None:
    ...
    cfg = cfg_override or load_cache_train_config(
        args.config or None,
