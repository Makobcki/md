...
class MMDiTFlowModel(nn.Module):
    ...
    def _patch_image_like_base(
        ...
        *,
        ...
        if value.shape[-2:] != expected_hw:
            raise ValueError(
                f"conditioning latent/mask spatial shape {tuple(value.shape[-2:])} must match target {expected_hw}."
            )
        if value.shape[-2] % embed.patch_size != 0 or value.shape[-1] % embed.patch_size != 0:
            raise ValueError(
                f"conditioning spatial shape must be divisible by patch_size={embed.patch_size}."
            )
    ...
    def _patch_image_like(
        ...
        *,
        ...
        tokens, grid_hw = self._patch_image_like_base(
            embed, value, device=device, dtype=dtype, expected_hw=expected_hw
        )
        return tokens + type_token, grid_hw

    def _control_type_ids(
        self,
        control_type: torch.Tensor | str | Sequence[str] | None,
        *,
        batch: int,
        streams: int,
        device: torch.device,
        ...
        if torch.is_tensor(control_type):
            ...
            if ids.dim() == 0:
                ...
                if ids.numel() == batch:
                    ...
                    raise ValueError(
                        "control_type tensor must have batch, stream or scalar length."
                    )
            elif ids.dim() == 2:
                if ids.shape == (batch, streams):
                    pass
                elif ids.shape == (batch, 1) or ids.shape == (1, streams):
                    ids = ids.expand(batch, streams)
                else:
                    raise ValueError(
                        f"control_type tensor shape {tuple(ids.shape)} must broadcast to {(batch, streams)}."
                    )
            else:
        ...
        if isinstance(control_type, str):
            ...
            if len(seq) == batch == streams and batch > 1 and all(isinstance(x, str) for x in seq):
                raise ValueError(
                    "control_type string list is ambiguous when batch size equals stream count; use a tensor or nested list."
                )
            if len(seq) == batch and all(isinstance(x, str) for x in seq):
                ...
                raise ValueError(
                    "control_type must be a string, batch list, stream list, nested list, or tensor ids."
                )
        try:
            ...
            raise ValueError(
                f"Unsupported control_type {exc.args[0]!r}; allowed: {allowed}."
            ) from exc
    ...
    def _row_stream_mask(
        self, names: list[str], present_for: set[str], *, tokens: int, device: torch.device
    ) -> torch.Tensor:
        rows = torch.tensor(
            [name == "mixed" or name in present_for for name in names],
            device=device,
            dtype=torch.bool,
        ).view(-1, 1)
    ...
    def _condition_tokens(
        ...
        if x.shape[-2] % self.cfg.patch_size != 0 or x.shape[-1] % self.cfg.patch_size != 0:
            raise ValueError(
                f"target latent shape {tuple(x.shape[-2:])} must be divisible by patch_size={self.cfg.patch_size}."
            )
        target = self.patch_embed(x)
        grid_hw = (x.shape[-2] // self.cfg.patch_size, x.shape[-1] // self.cfg.patch_size)
        target = (
            add_2d_pos_embed(target, grid_hw, self.cfg.pos_embed)
            + self.type_image
            + self.type_target
        )
        ...
        if len(task_names) != x.shape[0]:
            raise ValueError(
                f"task list length {len(task_names)} must match batch size {x.shape[0]}."
            )
        ...
        if source_latent is not None and m is not None and self.cfg.mask_as_source_channel:
            ...
            streams.append(
                (
                    tokens,
                    self._row_stream_mask(
                        task_names, {"img2img", "inpaint"}, tokens=tokens.shape[1], device=x.device
                    ),
                    cond_grid,
                    self.cfg.source_patch_size,
                )
            )
        ...
        if m is not None and not source_mask_fused:
            ...
            streams.append(
                (
                    tokens,
                    self._row_stream_mask(
                        task_names, {"inpaint"}, tokens=tokens.shape[1], device=x.device
                    ),
                    cond_grid,
                    self.cfg.mask_patch_size,
                )
            )
        if control_latents is not None:
            controls = (
                control_latents.unsqueeze(1) if control_latents.dim() == 4 else control_latents
            )
            if controls.dim() != 5:
            ...
            control_ids = self._control_type_ids(
                control_type, batch=x.shape[0], streams=controls.shape[1], device=x.device
            )
            ...
            control_present = torch.tensor(
                [name == "mixed" or name == "control" for name in task_names],
                device=x.device,
                dtype=torch.bool,
            ).view(-1, 1)
            ...
            for idx in range(controls.shape[1]):
                ...
                streams.append(
                    (
                        control_tokens,
                        control_present.expand(x.shape[0], control_tokens.shape[1]),
                        cond_grid,
                        self.cfg.control_patch_size,
                    )
                )
        if self.cfg.hierarchical_tokens_enabled:
            ...
            streams.append(
                (
                    coarse_tokens + self.type_image + self.type_coarse,
                    torch.ones(
                        (x.shape[0], coarse_tokens.shape[1]), device=x.device, dtype=torch.bool
                    ),
                    coarse_grid,
                    self.cfg.coarse_patch_size,
                )
            )
        ...
        streams.append(
            (
                target,
                torch.ones((x.shape[0], target.shape[1]), device=x.device, dtype=torch.bool),
                grid_hw,
                self.cfg.patch_size,
            )
        )
    ...
    def _project_text_tokens_with_mask(
        self, text: TextConditioning, *, device: torch.device, dtype: torch.dtype
    ) -> tuple[torch.Tensor, torch.Tensor | None]:
        raw = text.tokens.to(device=device)
        token_types = text.token_types
        if token_types is None:
            projected = (
                self.text_generic_in(raw.to(dtype=self.text_generic_in.weight.dtype))
                + self.type_text
            )
        else:
            ...
            if token_types.shape != raw.shape[:2]:
                raise ValueError(
                    f"text token_types shape {tuple(token_types.shape)} must match token shape {tuple(raw.shape[:2])}."
                )
            ...
            projected = torch.where(
                (token_types == 0).unsqueeze(-1),
                clip,
                torch.where((token_types == 1).unsqueeze(-1), t5, generic),
            )
    ...
    def _task_ids(
        self, task: str | Sequence[str], batch_size: int, device: torch.device
    ) -> torch.Tensor:
        ...
        try:
            ...
            raise ValueError(
                f"Unsupported MMDiT task {exc.args[0]!r}; allowed: {allowed}."
            ) from exc
    ...
    def forward(
        ...
        control_type: torch.Tensor | str | Sequence[str] | None = None,
        task: str | Sequence[str] = "txt2img",
        strength: torch.Tensor | float | None = None,
        control_strength: torch.Tensor | float | None = None,
    ) -> torch.Tensor:
        if x.dim() != 4:
        ...
        img, img_mask, target_tokens, grid_hw, rope_sections = self._condition_tokens(
            x, source_latent, mask, control_latents, control_type, control_strength, task=task
        )
        txt, txt_mask = self._project_text_tokens_with_mask(text, device=x.device, dtype=img.dtype)
        pooled = self.pooled_in(
            text.pooled.to(device=x.device, dtype=self.pooled_in.weight.dtype)
        ).to(dtype=img.dtype)
        ...
        if self.cfg.strength_embed:
            ...
            strength_v = self._scalar_batch(
                strength, default=1.0, batch=x.shape[0], device=x.device, dtype=img.dtype
            )
            if control_latents is None:
                control_v = torch.zeros(x.shape[0], device=x.device, dtype=img.dtype)
            else:
                control_v = self._scalar_batch(
                    control_strength,
                    default=1.0,
                    batch=x.shape[0],
                    device=x.device,
        ...
        for idx, block in enumerate(all_blocks):
            if self._block_mode(idx) == "image_only":
                img, txt = self._run_block(
                    block, img, txt, cond, None, img_mask, grid_hw, use_rope_sections, rope_sections
                )
            else:
                img, txt = self._run_block(
                    block,
